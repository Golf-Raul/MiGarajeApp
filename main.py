...
class Bloque(BoxLayout):
    def __init__(self, titulo, **kwargs):
        super().__init__(orientation='vertical', size_hint_y=None, spacing=5, padding=15, **kwargs)
        self.bind(minimum_height=self.setter('height'))
        self.add_widget(Label(text=titulo.upper(), size_hint_y=None, height=40, 
                              bold=True, color=(0.1, 0.4, 0.7, 1), font_size=16, halign="left"))

class PantallaBase(Screen):
    def crear_contenedor(self, titulo_pantalla):
        layout_principal = BoxLayout(orientation='vertical')
        cabecera = Label(text=titulo_pantalla, size_hint_y=None, height=70, bold=True, font_size=22, color=(1,1,1,1))
        ...
        self.content = BoxLayout(orientation='vertical', size_hint_y=None, spacing=20, padding=15)
    ...
    def input_est(self, hint, multi=False, alto=60):
        return TextInput(hint_text=hint, multiline=multi, size_hint_y=None, height=alto, font_size=18, padding=[10, 15])

    def check_est(self, texto):
        box = BoxLayout(orientation='horizontal', size_hint_y=None, height=50)
        box.add_widget(Label(text=texto, color=(0.2, 0.2, 0.2, 1), font_size=16))
        cb = CheckBox(color=(0.1, 0.4, 0.7, 1), size_hint_x=None, width=60)
        box.add_widget(cb)
        return box, cb
...
class PaginaUno(PantallaBase):
    def __init__(self, **kwargs):
        ...
        btn = Button(text="CONTINUAR", size_hint_y=None, height=80, background_color=(0.1, 0.7, 0.3, 1), bold=True)
        btn.bind(on_press=lambda x: self.ir_sig()); self.content.add_widget(btn)
        btn_h = Button(text="VER HISTORIAL", size_hint_y=None, height=60, background_color=(0.5, 0.5, 0.5, 1))
...
class PaginaDos(PantallaBase):
    def __init__(self, **kwargs):
        ...
        btn = Button(text="SIGUIENTE", size_hint_y=None, height=80, background_color=(0.1, 0.7, 0.3, 1)); btn.bind(on_press=lambda x: self.ir_sig())
...
class PaginaCuatro(PantallaBase):
    def __init__(self, **kwargs):
        ...
        self.ave = self.input_est("Observaciones/Averías", multi=True, alto=120); b_fin.add_widget(self.ave)
        self.cos = self.input_est("Coste Total (€)"); b_fin.add_widget(self.cos)
        self.content.add_widget(b_ext); self.content.add_widget(b_fin)
        btn = Button(text="GUARDAR FICHA", size_hint_y=None, height=90, background_color=(0, 0.6, 0.2, 1), bold=True)
...
class PantallaHistorial(PantallaBase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.l = self.crear_contenedor("HISTORIAL DE FICHAS")
        self.buscador = self.input_est("Buscar por Matrícula")
        self.buscador.bind(text=self.cargar_datos)
        self.content.add_widget(self.buscador)
        self.lista_resultados = BoxLayout(orientation='vertical', size_hint_y=None, spacing=10)
        ...
        btn_v = Button(text="VOLVER", size_hint_y=None, height=60, background_color=(0.7, 0.1, 0.1, 1))
    ...
    def cargar_datos(self, instance, value):
        ...
        for r in c.fetchall():
            btn = Button(text=f"{r[2]} - {r[0]} ({r[1]}) - {r[3]}€", size_hint_y=None, height=50)
