...
class PaginaCuatro(PantallaBase):
    ...
    def sig(self, x):
        App.get_running_app().datos.update({'r_del':self.r_del.text,'r_tra':self.r_tra.text,'frenos':self.frenos.get_val(),'pastillas':self.pastillas.get_val(),'luces':self.luces.get_val()})
